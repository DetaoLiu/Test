...
X = tf.placeholder(tf.float32, shape=[None, 784])  # X表示真的样本(即真实的手写数字),placeholder相当于定义了一个位置，这个位置中的数据在程序运行时再指定。

D_W1 = tf.Variable(xavier_init([784, 128]))  # 表示使用xavier方式初始化(一种参数随机初始化的方法)的判别器的D_W1参数，是一个784行128列的矩阵,Variable用于存放网络的权重矩阵
...
Z = tf.placeholder(tf.float32, shape=[None, 100])  # Z表示生成器的输入(在这里是噪声)，是一个N行100列的矩阵
...
def sample_Z(m, n):  # 生成维度为[m, n]的随机噪声作为生成器G的输入
    return np.random.uniform(-1., 1., size=[m, n])  #随机生成一个服从均匀分布的矩阵


def generator(z):  # 生成器，z的维度为[N, 100]
    G_h1 = tf.nn.relu(tf.matmul(z, G_W1) + G_b1)  # 输入的随机噪声乘以G_W1矩阵加上偏置G_b1，G_h1维度为[N, 128]，tf.nn.relu()函数是将大于0的数保持不变，小于0的数置为0
    G_log_prob = tf.matmul(G_h1, G_W2) + G_b2  # G_h1乘以G_W2矩阵加上偏置G_b2，G_log_prob维度为[N, 784]，是一个神经元的输入
    G_prob = tf.nn.sigmoid(G_log_prob)  # G_log_prob经过一个sigmoid函数，G_prob维度为[N, 784]，是经过一个激活函数后的输出
...
def discriminator(x):  # 判别器，x的维度为[N, 784]
    ...
    return D_prob, D_logit  # 返回D_prob（判断图片的真假）, D_logit
...
D_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=D_logit_real, labels=tf.ones_like(
    D_logit_real)))  # 对判别器对真实样本的判别结果计算误差(将结果与1比较)，tf.ones_like(D_logit_real)创建维度和D_logit_real相等的全是1的标注，真实图片
